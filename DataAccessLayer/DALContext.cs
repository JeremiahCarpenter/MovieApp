...
namespace DataAccessLayer
{
    class Mapper //parent
    {
        ...
        public void assert(bool test, string message) //used to make sure something is true
        {
            if (!test)
            {
    ...
    {
        int CategoryIDOrdinal; //ordinal is the column offset
        int CategoryOrdinal;
        public CategoryMapper(SqlDataReader r) //ctor used only once to validate the correct column numbers
        {
    ...
    {
        int RoleIDOrdinal; //ordinal is the column offset
        int RoleOrdinal;
        public RoleMapper(SqlDataReader r) //ctor used only once to validate the correct column numbers
        {
        ...
        public RoleDAL ToRole(SqlDataReader r) //different from ctor used many times in loops
        {
    ...
    class UserMapper : Mapper //maps Users
    {
        int UserIDOrdinal; //ordinal is the column offset
        int UserNameOrdinal;
        int EmailOrdinal;
        int PasswordOrdinal;
        int RoleIDOrdinal;
        ...
        int RoleOrdinal;
        public UserMapper(SqlDataReader r) //ctor used only once to validate the correct column numbers
        {
    ...
    {
        int MovieIDOrdinal;
        int TitleOrdinal; //ordinal is the column offset
        int LengthOrdinal;
        int RatingOrdinal;
        ...
        public MovieMapper(SqlDataReader r) //ctor used only once to validate the correct column numbers
        {
        ...
        public MovieDAL ToMovie(SqlDataReader r) //different from ctor used many times in loops
        {
    ...
    class MovieUserMapper : Mapper
    {
        ...
        int MovieIDOrdinal; //ordinal is the column offset
        int UserIDOrdinal;
        int DateOrdinal;
        ...
        public MovieUserMapper(SqlDataReader r)  //ctor used only once to validate the correct column numbers
        {
        ...
        public MovieUserDAL ToMovieUser(SqlDataReader r)  //different from ctor used many times in loops
        {
    ...
    {
        public void Dispose() //lets us use the Using pattern
        {
            Con.Dispose();
        }

        private System.Data.SqlClient.SqlConnection Con; //shortens down to con

        private void Log(Exception ex) //puts the exceptions into logger
        {
            Logger.Logger.Log(ex);
        }

        public ContextDAL() //ctor
        {
        ...
        private void EnsureConnected() //used anytime connection to database is required
        {
            if (Con.State != System.Data.ConnectionState.Open) //opens con if broken
            {
        ...
        public SqlDataReader ExecuteCommand(SqlCommand cmd) //testing purposes only
        {
        ...
        // impliments crud/stored procedures to be used elsewhere in solution for categories
        #region Categories
        ...
        // impliments crud/stored procedures to be used elsewhere in solution for roles
        #region Roles
        ...
        #endregion Roles
        //  impliments crud/stored procedures to be used elsewhere in solution for movies
        #region Movies
        ...
        #endregion Movies
        // impliments crud/stored procedures to be used elsewhere in solution for users
        #region Users
        public int CreateUser(string UserName, string Email, string Password, int RoleID)
        {
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand("CreateUser", Con))
                {
                    ...
                    command.ExecuteNonQuery(); //explain

                    rv = Convert.ToInt32(command.Parameters["@UserID"].Value); //id#
                }
            }
            catch (Exception ex)
            {
        ...
        public UserDAL FindUserByID(int UserID)
        {
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand("FindUserByID", Con))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        UserMapper mapper = new UserMapper(reader); //call once
                        if (reader.Read()) //retruns one record
                        {
        ...
        public List<UserDAL> GetUsers(int skip, int take)
        {
            ...
            try
            {
                EnsureConnected(); //establish con is up
                using (SqlCommand command = new SqlCommand("GetUsers", Con))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader()) //returns records
                    {
                        UserMapper mapper = new UserMapper(reader); //outside loop
        ...
        #endregion Users
        // impliments crud/stored procedures to be used elsewhere in solution for movieusers
        #region MovieUsers
